...
class Kiehls(Website):
    ...
    def get_urls(self):
        ...
        return [product_url for product_url in self._get_product_urls(hrefs)]

    def is_valid(self, url):
        return self._is_valid_product_url(url)

    def scrape_product_url(self, product_urls):
        products = []
    ...
    def _is_valid_product_url(self, url):
        return not not make_soup(url).find_all('div', class_='l-product_details-wrapper')
    ...
    def _scrape_product(self, url):
        ...
        # not sure why there could be multiple product names??
